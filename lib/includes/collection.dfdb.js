            ...
            if (typeof doc !== 'object' || Array.isArray(doc)) {
                ...
                if (this.hasSchema()) {
                    ...
                    if (this._schemaValidator(doc)) {
                        ...
                        this._lastError = `${constants_dfdb_1.Errors.SchemaDoesntApply}. '\$${this._schemaValidator.errors[0].dataPath}' ${this._schemaValidator.errors[0].message}`;
                    }
                }
                //
            ...
            if (this._connected) {
                ...
                if (schemaMD5 !== this._manifest.schemaMD5) {
                    ...
                    try {
                        ...
                        this._lastError = `${constants_dfdb_1.Errors.InvalidSchema}. '\$${ajv.errors[0].dataPath}' ${ajv.errors[0].message}`;
                    }
                    //
                    // Is it valid?
                    if (valid) {
            ...
            Object.keys(this._data).forEach((id) => {
                if (!this.error()) {
                    if (!validator(this._data[id])) {
                        this._lastError = `${constants_dfdb_1.Errors.SchemaDoesntApply}. Id: ${id}. '\$${validator.errors[0].dataPath}' ${validator.errors[0].message}`;
                    }
                }
            });
            //
