...
class Collection {
    ...
    search(conditions, done) {
        ...
        if (Object.keys(indexedConditions).length > 0) {
            this.findIds(indexedConditions, (ids) => {
    ...
    searchOne(conditions, done) {
        if (typeof done === null) {
            done = (finding) => { };
        }
        this.search(conditions, (findings) => {
            if (findings.length > 0) {
                done(findings[0]);
            }
            else {
