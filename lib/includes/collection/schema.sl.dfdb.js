            ...
            if (this._mainObject._connected) {
                ...
                if (schemaMD5 !== this._mainObject._manifest.schemaMD5) {
                    ...
                    try {
                        ...
                        this._mainObject._subLogicErrors.setLastRejection(new rejection_dfdb_1.Rejection(rejection_codes_dfdb_1.RejectionCodes.InvalidSchema, `'\$${ajv.errors[0].dataPath}' ${ajv.errors[0].message}`));
                    }
                    //
                    // Is it valid?
                    if (valid) {
            ...
            Object.keys(this._mainObject._data).forEach((id) => {
                if (!this._mainObject.error()) {
                    if (!validator(this._mainObject._data[id])) {
                        this._mainObject._subLogicErrors.setLastRejection(new rejection_dfdb_1.Rejection(rejection_codes_dfdb_1.RejectionCodes.SchemaDoesntApply, `Id: ${id}. '\$${validator.errors[0].dataPath}' ${validator.errors[0].message}`));
                    }
                }
            });
            //
